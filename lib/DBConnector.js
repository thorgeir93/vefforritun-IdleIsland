...
module.exports.setGameState=function storeGameState(userN, gameS, score, cb){
  pg.connect(DATABASE, function (error, client, done) {
    ...
    client.query(query, values, function (err, result) {
      ...
      if (err) {
        console.log('það kom upp error við að setja inn gamestate')
        return cb(error);
      } else {
        ...
        client.query(query, values, function (err, result) {
          ...
          if (err) {
                    console.log('það kom upp error við að setja inn score')

            return cb(error);
          } else {
                    console.log('allt gekk vel nú eiga að vera komin ný gögn í töfluna')
